...
pub struct MiniPlaylistPlayback {
            ...
            egui::ScrollArea::horizontal()
                ...
                .show(ui, |ui| {
                    for track in playlist.iter() {
                        if !search.is_empty()
                            && search.len() >= 3
                            && !track.title.to_ascii_lowercase().contains(&search)
                            && !track.artist.to_ascii_lowercase().contains(&search)
                        {
                            continue;
                        }

                        let is_current = current.as_ref().is_some_and(|c| c.path == track.path);
                        ...
                        let (fill, text_col) = if is_current {
                            (sel_bg, sel_fg)
                        } else {
                            (bg, fg)
                        };
                        ...
                        let resp = ui.add(
                            egui::Button::selectable(
                                is_current,
                                egui::RichText::new(label).color(text_col),
                            )
                            .fill(fill),
                        );
